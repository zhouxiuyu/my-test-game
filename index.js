...
;(function () {
  ...
  Runner.prototype = {
    ...
    handleEvent: function (e) {
      return function (evtType, events) {
        if (window.playdeck.popupIsOpen()) {
          return
        }
        switch (evtType) {
          case events.KEYDOWN:
  ...
  DistanceMeter.prototype = {
    ...
    setHighScore: function (distance) {
      ...
      // 🔵 set high score
      window.playdeck.setScore(distance, false)
    },
...
window.addEventListener('load', (e) => {
  ...
  playdeck.loading()
  setTimeout(() => playdeck.loading(100), 4000)

  console.log('🎾 game:', JSON.stringify(playdeck.getUser()))

  playdeck.getScore().then((result) => {
    console.log('🎾 game:', 'getScore()', JSON.stringify(result))
  })

  playdeck.getData('numOfGames').then((result) => {
    console.log('🎾 game: ', `getData(numOfGames) -> `, result)
  })

  playdeck.getData('B').then((result) => {
    console.log('🎾 game: ', `getData(B) -> `, result)
  })

  window.playdeck.setData('numOfGames', '1').then((result) => {
    // console.log('🎾 game: ', `setData(numOfGames, 1) -> `, result)
  })

  window.playdeck.setData('B', '2').then((result) => {
    // console.log('🎾 game: ', `setData(B, 2) -> `, result)
  })

  playdeck.addGameListener((event) => {
  ...
  console.log('🎾 game: ', `user locale: ${playdeck.getUserLocale()}`)
})
